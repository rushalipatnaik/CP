...
ll dis;
if(xa==xb && xa!=xf) dis=abs(xa-xb);
else if(ya==yb && ya!=yf) dis=abs(ya-yb);
else if(xa==xb==xf) dis=abs(ya-yb)+2;
else if(ya==yb==yf) dis=abs(xa-xb)+2;
else if(xa!=xb!=xf) dis=abs(xa-xb)+abs(ya-yb);
cout<<dis<<endl;
}
